...
class Printer:
    ...
    def __call__(self, *args, **kwargs):
        if self.verboseness:
            key = next(self.verbose_key) if isinstance(self.verbose_key, types.GeneratorType) else self.verbose_key
            if self.verbosity == 0:
                print()
...
class Timer:
    def __init__(self, name='timer', color='blue', level=0, verbose=False, print_details=False, print_sub=True):
        self.name = name
        self.color = color
        self.level = level
        self.zero = time.time()
    ...
    def __call__(self, string=None, t=None):
        if string is not None:
            self.print_time(self.level * '  ' + string, t=(time.time() - self.last) if t is None else t, end='\n\n' if self.level==0 else '\n')
        self.last = time.time()
    ...
    def start(self, name):
        if self.sub is not None:
            ...
            self.sub = Timer(name=name, color=self.color, level=self.level + 1,
                             verbose=self.verbose, print_details=self.print_details)
                ...
                if isinstance(v, dict):
                    ...
                    if 'n' in v:
                        ...
                        if self.print_details:
                            self.cprint(f', calls = {n}' + ', avg = %.6f' % (t/n), end='')
                    self.cprint(')')
                    if 'rec' in v:

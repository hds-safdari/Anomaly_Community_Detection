...
class AnomalyDetection:
	...
	def _QIJ(self, data, data_T_vals, subs_nz):
		...
		if isinstance(data, skt.dtensor):
			...
			nz_recon_I =  self.mupr * poisson.pmf(data.vals, self.pibr) * poisson.pmf(data_T_vals, self.pibr)
	...
	def _ELBO(self, data, data_T, mask=None,subs_nz_mask=None):
		...
		if self.flag_anomaly == False:
			...
			if self.pibr >= 0:
				if mask is None:
					l += np.log(self.pibr+EPS) * (self.Qij_dense[data.subs] * data.vals).sum()
				else:
					subs_nz = np.logical_and(mask > 0,Adense > 0)
					l += np.log(self.pibr+EPS) * (self.Qij_dense[subs_nz] * (Adense[subs_nz])).sum()
